              ...
              <table className="w-full table-auto border-collapse min-w-[600px]">
                ...
                <thead>
                  <tr className="bg-black/40 text-[10px] font-black text-gray-600 uppercase tracking-[0.1em] sm:tracking-[0.2em]">
                    <th className="px-1.5 sm:px-2 py-3 text-left">#</th>
                    <th className="px-1.5 sm:px-2 py-3 text-left">Pair</th>
                    <th className="px-1.5 sm:px-2 py-3 text-left">Result</th>
                    <th className="px-1.5 sm:px-2 py-3 text-left">Stake ($)</th>
                    <th className="px-1.5 sm:px-2 py-3 text-right">Return ($)</th>
                    <th className="px-1.5 sm:px-2 py-3 text-right">Balance ($)</th>
                    <th className="px-1.5 sm:px-2 py-3 text-center">Payout (%)</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800/30">
                  {results.computedTrades.map((trade, i) => (
                    <tr key={i} className="hover:bg-white/[0.02] transition-colors group">
                      <td className="px-1.5 sm:px-2 py-3 text-xs font-black text-gray-600">{i + 1}</td>
                      <td className="px-1.5 sm:px-2 py-3">
                        <input
                          type="text"
                          value={trade.pair || ''}
                          onChange={(e) => handleUpdateTrade(i, 'pair', e.target.value)}
                          placeholder="EUR/USD"
                          className={`w-full bg-black/40 border border-gray-800 rounded-lg px-2 py-1.5 text-[11px] font-black focus:border-[#D4AF37] outline-none ${
                      ...
                      <td className="px-1.5 sm:px-2 py-3">
                        <input
                          ...
                          placeholder="1.00"
                          className={`w-full bg-black/40 border border-gray-800 rounded-lg px-2 py-1.5 text-xs font-black focus:border-[#D4AF37] outline-none ${
                            trade.amount > 0 ? 'text-white' : 'text-gray-600'
                          }`}
                        />
                      </td>
                      <td className="px-1.5 sm:px-2 py-3 text-right font-black text-xs">
                        {trade.result === 'WIN' ? (
                      ...
                      <td className="px-1.5 sm:px-2 py-3 text-right font-black text-xs text-[#D4AF37]">
